...
# class HivSelfSupBaseClass(InMemoryDataset):
...
class HivSelfSupPhysicalDataSet(InMemoryDataset):
    """Data set of whole slide using physical contact as edges"""

    def __init__(self, root=ProcessSlideFile.FOLDER_DIR.parent.as_posix(), transform=None, pre_transform=None,
                 pre_filter=None):
        super(HivSelfSupPhysicalDataSet, self).__init__(root, transform, pre_transform, pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])
...
class HivSelfSupRadiusDataSet(InMemoryDataset):
    """Data set of whole slide using distance as edged with a cutoff of radius"""

    def __init__(self, root=ProcessSlideFile.FOLDER_DIR.parent.as_posix(), transform=None, pre_transform=None,
                 pre_filter=None, radius=25.0):
        self.radius = radius
        super(HivSelfSupRadiusDataSet, self).__init__(root, transform, pre_transform, pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])
