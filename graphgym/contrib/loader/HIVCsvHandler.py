...
class ProcessSlideFile(object):
    ...
    @property
    def pos_ten(self) -> Tensor:
        if self._pos_ten is None:
            self._set_pos_ten()
        return self._pos_ten
    ...
    def _set_dist_mat(self):
        self._dist_mat = np.exp(-distance_matrix(self.pos_ten.numpy(), self.pos_ten.numpy()) ** 2)

    def _set_pos_ten(self):
        locations_normalized = self.file_pointer.loc[:, [self._X_POS_COL_NAME, self._Y_POS_COL_NAME]].values \
                               / ProcessSlideFile._POS_NORMALIZATION_FACTOR
        self._pos_ten = torch.tensor(locations_normalized)
